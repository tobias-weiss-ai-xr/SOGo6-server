...
"""Coverage tests (sogo-cov-56) for the Sieve/Filter/Snooze pipeline.

Covers the remaining branches of ClientSieve (Sieve script compilation,
ManageSieve client wrapper, vacation/forward/notification/filter script
builders, capability negotiation), ModuleFilter (granular filter ops, rule
ordering, set_section persist semantics) and ModuleSnooze (snooze recording,
re-schedule/unsnooze, due/expired listing)."""
...
# ============================================================================ #
# ModuleFilter — coverage of the remaining branches                            #
# ============================================================================ #

from app.module.mail.ModuleFilter import ModuleFilter  # noqa: E402
from app.utils import errors as err
from app.utils.exceptions import BugException, RequestException
from app.utils.constants import FILTER_SECTION_FILTERS, FILTER_SECTION_NOTIFICATION


class _FakeDb:
    """Stand-in for ClientSQL/ClientMySQL surface used by ModuleFilter."""

    def __init__(self, filters_value=None, user_exists=True, update_affected=1):
        self.filters_value = filters_value
        self.user_exists = user_exists
        self.update_affected = update_affected
        self.monitoring = {"connects": 0, "updates": [], "selects": []}

    def connect(self):
        self.monitoring["connects"] += 1

    def select_from_table(self, table_name, column_tuple, condition):
        self.monitoring["selects"].append((table_name, column_tuple))
    ...
    def update_in_table(self, table_name, column_tuple, values_list, condition):
        self.monitoring["updates"].append(values_list[0])
        return self.update_affected


def _make_module(fake_db):
    ...
    mail_settings = SimpleNamespace(
        ...
        get_mail_filtering_settings_for_type=lambda t: {"host": "sieve.example.org", "port": 4190},
    )
    process_settings = SimpleNamespace(SOGO_P_DB_TYPE="MySQL", get_db_settings=lambda: {})
    with mock.patch(
...
class _FakeSieveClient:
    """Minimal authenticated filtering client used by ModuleFilter._open_filtering_client."""

    def __init__(self, activated=None):
        self.activated = activated if activated is not None else {}
        self.connect_calls = 0
        self.login_args = None
        self.logout_calls = 0
        self.set_merged_calls = 0
    ...
    def set_merged_filters(self, filters_config):
        self.set_merged_calls += 1
...
class TestModuleFilterConfig:
    def test_get_filtering_conf_happy(self):
        module = _make_module(_FakeDb(filters_value={"filters": []}))
        conf = module._get_filtering_conf()
        assert conf["type"] == "sieve"
        assert conf["args"] == {"host": "sieve.example.org", "port": 4190}

    def test_get_filtering_conf_unsupported_raises_bug(self):
        user = SimpleNamespace(uid="u")
        mail_settings = SimpleNamespace(SOGO_D_MAIL_FILTERING_TYPE="procmail")
        process = SimpleNamespace(SOGO_P_DB_TYPE="MySQL", get_db_settings=lambda: {})
        with mock.patch("app.module.mail.ModuleFilter.import_and_instantiate_manager", return_value=mock.Mock()):
            module = ModuleFilter(user, mail_settings, process)
        with pytest.raises(BugException):
            module._get_filtering_conf()

    def test_get_filtering_conf_unsupported_constructor_ok(self):
        # A non-registered type is only rejected on access, not in __init__.
        user = SimpleNamespace(uid="u")
        mail_settings = SimpleNamespace(SOGO_D_MAIL_FILTERING_TYPE="procmail")
        process = SimpleNamespace(SOGO_P_DB_TYPE="MySQL", get_db_settings=lambda: {})
        with mock.patch("app.module.mail.ModuleFilter.import_and_instantiate_manager", return_value=mock.Mock()):
            ModuleFilter(user, mail_settings, process)

    def test_open_filtering_client(self):
        fake_db = _FakeDb(filters_value={"filters": []})
        module = _make_module(fake_db)
        client = _FakeSieveClient()
        with mock.patch("app.module.mail.ModuleFilter.import_and_instantiate_manager", return_value=client):
            out = module._open_filtering_client()
        assert out is client
        assert client.connect_calls == 1
        assert client.login_args == ("testuser@example.org", "pw")

    def test_open_filtering_client_uses_login_mail_filtering(self):
        user = SimpleNamespace(uid="uid@x", login_mail_filtering="sieve-user", password="pw")
        mail_settings = SimpleNamespace(
            SOGO_D_MAIL_FILTERING_TYPE="sieve",
            get_mail_filtering_settings_for_type=lambda t: {},
        )
        process = SimpleNamespace(SOGO_P_DB_TYPE="MySQL", get_db_settings=lambda: {})
        fake_db = _FakeDb(filters_value={"filters": []})
        with mock.patch("app.module.mail.ModuleFilter.import_and_instantiate_manager", return_value=fake_db):
            module = ModuleFilter(user, mail_settings, process)
        client = _FakeSieveClient()
        with mock.patch("app.module.mail.ModuleFilter.import_and_instantiate_manager", return_value=client):
            module._open_filtering_client()
        assert client.login_args == ("sieve-user", "pw")


class TestModuleFilterDbHelpers:
    def test_read_current_filters_missing_row_raises(self):
        fake_db = _FakeDb(filters_value=None, user_exists=False)
        module = _make_module(fake_db)
        with pytest.raises(RequestException) as ex:
            module._read_current_filters()
        assert ex.value.error == err.ERROR_USER_PROFILE_NOT_FOUND

    def test_read_current_filters_none_returns_empty(self):
        fake_db = _FakeDb(filters_value=None)
        module = _make_module(fake_db)
        assert module._read_current_filters() == {}

    def test_read_current_filters_value(self):
        fake_db = _FakeDb(filters_value={"filters": [{"name": "a"}]})
        module = _make_module(fake_db)
        out = module._read_current_filters()
        assert out["filters"] == [{"name": "a"}]

    def test_write_filters_succeeds(self):
        fake_db = _FakeDb(filters_value={"filters": []})
        module = _make_module(fake_db)
        module._write_filters({"filters": []})
        assert fake_db.monitoring["updates"] == [{"filters": []}]

    def test_write_filters_noop_with_existing_row_is_success(self):
        fake_db = _FakeDb(filters_value={"filters": []}, update_affected=0)
        module = _make_module(fake_db)
        module._write_filters({"filters": []})
        assert fake_db.monitoring["selects"]  # existence probe ran

    def test_write_filters_missing_row_raises(self):
        fake_db = _FakeDb(filters_value=None, user_exists=False, update_affected=0)
        module = _make_module(fake_db)
        with pytest.raises(RequestException) as ex:
            module._write_filters({"filters": []})
        assert ex.value.error == err.ERROR_USER_PROFILE_UPDATE_FAILED


class TestModuleFilterGranular:
    def test_get_section(self):
        fake_db = _FakeDb(filters_value={"vacation": {"enabled": True}})
        module = _make_module(fake_db)
        assert module.get_section("vacation") == {"enabled": True}
        assert module.get_section("forward") is None
        assert fake_db.monitoring["connects"] == 2

    def test_get_filter_found(self):
        fake_db = _FakeDb(filters_value={"filters": [{"name": "Spam"}]})
        module = _make_module(fake_db)
        out = module.get_filter("spam")
        assert out == {"name": "Spam"}

    def test_get_filter_not_found(self):
        fake_db = _FakeDb(filters_value={"filters": [{"name": "Spam"}]})
        module = _make_module(fake_db)
        with pytest.raises(RequestException) as ex:
            module.get_filter("nope")
        assert ex.value.error == err.ERROR_FILTER_NOT_FOUND

    def test_get_filter_no_filters_section(self):
        fake_db = _FakeDb(filters_value={})
        module = _make_module(fake_db)
        with pytest.raises(RequestException) as ex:
            module.get_filter("x")
        assert ex.value.error == err.ERROR_FILTER_NOT_FOUND

    def test_set_filter_new_appends(self):
        fake_db = _FakeDb(filters_value={"filters": [{"name": "a"}]})
        module = _make_module(fake_db)
        client = _FakeSieveClient(activated={FILTER_SECTION_FILTERS: True})
        with mock.patch.object(module, "_open_filtering_client", return_value=client):
            out = module.set_filter("b", {"name": "b", "enabled": True})
        names = [f["name"] for f in out["filters"]]
        assert names == ["a", "b"]
        assert client.logout_calls == 1

    def test_set_filter_replaces_case_insensitive(self):
        fake_db = _FakeDb(filters_value={"filters": [{"name": "Spam", "enabled": True}]})
        module = _make_module(fake_db)
        client = _FakeSieveClient(activated={FILTER_SECTION_FILTERS: True})
        with mock.patch.object(module, "_open_filtering_client", return_value=client):
            out = module.set_filter("spam", {"name": "spam", "enabled": False})
        assert len(out["filters"]) == 1
        assert out["filters"][0]["enabled"] is False

    def test_delete_filter(self):
        fake_db = _FakeDb(filters_value={"filters": [{"name": "a"}, {"name": "b"}]})
        module = _make_module(fake_db)
        client = _FakeSieveClient(activated={FILTER_SECTION_FILTERS: True})
        with mock.patch.object(module, "_open_filtering_client", return_value=client):
            out = module.delete_filter("a")
        assert [f["name"] for f in out["filters"]] == ["b"]

    def test_delete_filter_not_found(self):
        fake_db = _FakeDb(filters_value={"filters": [{"name": "a"}]})
        module = _make_module(fake_db)
        with pytest.raises(RequestException) as ex:
            module.delete_filter("zz")
        assert ex.value.error == err.ERROR_FILTER_NOT_FOUND

    def test_reorder_filters(self):
        fake_db = _FakeDb(filters_value={"filters": [{"name": "a"}, {"name": "b"}, {"name": "c"}]})
        module = _make_module(fake_db)
        client = _FakeSieveClient(activated={FILTER_SECTION_FILTERS: True})
        with mock.patch.object(module, "_open_filtering_client", return_value=client):
            out = module.reorder_filters(["c", "a"])
        assert [f["name"] for f in out["filters"]] == ["c", "a", "b"]

    def test_reorder_filters_missing_name_raises(self):
        fake_db = _FakeDb(filters_value={"filters": [{"name": "a"}]})
        module = _make_module(fake_db)
        with pytest.raises(RequestException) as ex:
            module.reorder_filters(["a", "zz"])
        assert ex.value.error == err.ERROR_FILTER_NOT_FOUND

    def test_reorder_filters_non_dict_entries_skipped(self):
        fake_db = _FakeDb(filters_value={"filters": ["junk", {"name": "a"}]})
        module = _make_module(fake_db)
        client = _FakeSieveClient(activated={FILTER_SECTION_FILTERS: True})
        with mock.patch.object(module, "_open_filtering_client", return_value=client):
            out = module.reorder_filters(["a"])
        assert out["filters"] == [{"name": "a"}, "junk"]

    def test_push_to_sieve(self):
        current = {
            "filters": [{"name": "a", "enabled": True}],
            "vacation": {"enabled": True},
        }
        fake_db = _FakeDb(filters_value=current)
        module = _make_module(fake_db)
        client = _FakeSieveClient(activated={FILTER_SECTION_FILTERS: True, "vacation": True})
        with mock.patch.object(module, "_open_filtering_client", return_value=client):
            out = module.push_to_sieve()
        assert out is current
        assert client.set_merged_calls == 2  # filters + vacation sections


class TestModuleFilterIsSectionEnabled:
    def test_list_with_enabled(self):
        assert ModuleFilter._is_section_enabled([{"enabled": True}, {"enabled": False}]) is True

    def test_list_all_disabled(self):
        assert ModuleFilter._is_section_enabled([{"enabled": False}]) is False

    def test_list_empty(self):
        assert ModuleFilter._is_section_enabled([]) is False

    def test_list_non_dict_items(self):
        # items without 'enabled' default to enabled=True; non-dict items skipped
        assert ModuleFilter._is_section_enabled(["junk", {"name": "x"}]) is True
        assert ModuleFilter._is_section_enabled(["junk"]) is False

    def test_dict_enabled(self):
        assert ModuleFilter._is_section_enabled({"enabled": True}) is True

    def test_dict_disabled(self):
        assert ModuleFilter._is_section_enabled({"enabled": False}) is False

    def test_dict_missing_enabled(self):
        assert ModuleFilter._is_section_enabled({"enabled": 1}) is True
        assert ModuleFilter._is_section_enabled({"foo": "bar"}) is False

    def test_non_container(self):
        assert ModuleFilter._is_section_enabled("Vacation") is False
        assert ModuleFilter._is_section_enabled(None) is False


class TestModuleFilterSetSection:
    def test_set_section_persists_active(self):
        current = {"filters": [{"name": "a", "enabled": True}]}
        fake_db = _FakeDb(filters_value=current)
        module = _make_module(fake_db)
        client = _FakeSieveClient(activated={FILTER_SECTION_FILTERS: True})
        with mock.patch.object(module, "_open_filtering_client", return_value=client):
            out = module.set_section(FILTER_SECTION_FILTERS, [{"name": "a", "enabled": True}])
        assert out[FILTER_SECTION_FILTERS] == [{"name": "a", "enabled": True}]
        assert client.logout_calls == 1
        assert fake_db.monitoring["updates"]

    def test_set_section_disabled_still_persists(self):
        current = {"vacation": {}}
        fake_db = _FakeDb(filters_value=current)
        module = _make_module(fake_db)
        client = _FakeSieveClient(activated={"vacation": False})
        with mock.patch.object(module, "_open_filtering_client", return_value=client):
            out = module.set_section("vacation", {"enabled": False})
        assert out["vacation"] == {"enabled": False}
        assert fake_db.monitoring["updates"]

    def test_set_section_not_activated_not_persisted(self):
        current = {"notification": {"enabled": True, "notify_addresses": ["a@b.c"]}}
        fake_db = _FakeDb(filters_value=current)
        module = _make_module(fake_db)
        client = _FakeSieveClient(activated={})  # nothing activated on server
        with mock.patch.object(module, "_open_filtering_client", return_value=client):
            out = module.set_section(FILTER_SECTION_NOTIFICATION, {"enabled": True, "notify_addresses": ["a@b.c"]})
        # The section is popped before persisting -> stored column lacks it.
        assert FILTER_SECTION_NOTIFICATION not in out
        assert fake_db.monitoring["updates"]
        assert fake_db.monitoring["updates"][-1].get(FILTER_SECTION_NOTIFICATION) is None
        assert client.logout_calls == 1

    def test_set_section_error_propagates_and_logout(self):
        current = {"filters": []}
        fake_db = _FakeDb(filters_value=current)
        module = _make_module(fake_db)
        client = _FakeSieveClient()
        client.set_merged_filters = mock.Mock(side_effect=RequestException("sieve down", err.ERROR_SIEVE_COMMAND_FAILED))
        with mock.patch.object(module, "_open_filtering_client", return_value=client):
            with pytest.raises(RequestException):
                module.set_section(FILTER_SECTION_FILTERS, [])
        assert client.logout_calls == 1
        assert not fake_db.monitoring["updates"]  # error raised before persist

    def test_set_section_non_filter_section_skips_sieve(self):
        current = {"other_key": {}}
        fake_db = _FakeDb(filters_value=current)
        module = _make_module(fake_db)
        with mock.patch.object(module, "_open_filtering_client", side_effect=AssertionError("must not open client")):
            out = module.set_section("custom_section", {"data": 1})
        assert out == {"other_key": {}, "custom_section": {"data": 1}}


# ============================================================================ #
# ModuleSnooze — coverage of the remaining branches                            #
# ============================================================================ #

# NOTE: `datetime` must stay the *module* here (the ClientSieve section above uses
# `datetime.tzinfo` / `datetime.now`), so the class is imported under an alias.
from datetime import datetime as _DateTime, timezone as _dt_tz, timedelta

from app.module.mail.ModuleSnooze import ModuleSnooze, _to_utc


class _SnoozeDb:
    """ClientSQL stand-in with a queue of select results and insert/delete capture."""

    def __init__(self, select_results=None):
        self._results = list(select_results) if select_results is not None else []
        self.select_calls = []
        self.inserted = []
        self.deleted = []

    def select_from_table(self, table_name, column_tuple, condition):
        self.select_calls.append((table_name, column_tuple, condition))
        if self._results:
            return self._results.pop(0)
        return []

    def insert_in_table(self, table_name, column_tuple, values_tuple):
        self.inserted.append((table_name, column_tuple, values_tuple))
...
def _snooze_row(snooze_until=_UNSET, mail_uid="959", folder="INBOX", account_id="0", id_=1):
    if snooze_until is _UNSET:
        snooze_until = _DateTime(2030, 1, 1, 12, 0, 0, tzinfo=_dt_tz.utc)
    return [
        id_,
        ...
        folder,
        snooze_until,
        _DateTime(2026, 8, 30, 8, 38, 47, tzinfo=_dt_tz.utc),
        account_id,
    ]
...
class TestToUtc:
    def test_naive_is_treated_as_utc(self):
        dt = _to_utc("2026-08-31T08:38:47")
        assert dt.tzinfo is not None
        assert dt == _DateTime(2026, 8, 31, 8, 38, 47, tzinfo=_dt_tz.utc)

    def test_aware_is_converted(self):
        dt = _to_utc("2026-08-31T10:38:47+02:00")
        assert dt == _DateTime(2026, 8, 31, 8, 38, 47, tzinfo=_dt_tz.utc)
...
class TestRowToDict:
    def test_none_fields(self):
        ...
        row = [1, "u", "m", "INBOX", None, None, None, "0"]
        out = module._row_to_dict(row)
        assert out["original_folder"] is None
        assert out["snooze_until"] is None
        assert out["created_at"] is None

    def test_plain_value_str(self):
        db = _SnoozeDb()
        module = ModuleSnooze(db)
        row = [1, "u", "m", "INBOX", "INBOX", "2030-01-01", "x", 7]
        out = module._row_to_dict(row)
        assert out["snooze_until"] == "2030-01-01"
        assert out["created_at"] == "x"
        # account_id is stored raw (not _fmt-ed)
        assert out["account_id"] == 7


class TestSnooze:
    def test_past_snooze_until_raises(self):
        db = _SnoozeDb()
        module = ModuleSnooze(db)
        past = _DateTime.now(_dt_tz.utc) - timedelta(hours=1)
        with pytest.raises(RequestException) as ex:
            module.snooze("u", "0", "959", "INBOX", past)
        assert ex.value.error == err.ERROR_VALIDATION_FAILED
        assert not db.inserted

    def test_duplicate_raises(self):
        db = _SnoozeDb(select_results=[[_snooze_row()]])
        module = ModuleSnooze(db)
        future = _DateTime.now(_dt_tz.utc) + timedelta(days=1)
        with pytest.raises(RequestException) as ex:
            module.snooze("u", "0", "959", "INBOX", future)
        assert ex.value.error == err.ERROR_SNOOZE_DUPLICATE
        assert not db.inserted

    def test_success_returns_row_dict(self):
        row = _snooze_row(mail_uid="959")
        db = _SnoozeDb(select_results=[[], [row]])
        module = ModuleSnooze(db)
        future = _DateTime.now(_dt_tz.utc) + timedelta(days=1)
        out = module.snooze("u", "0", "959", "INBOX", future, original_folder="Archive")
        assert out["mail_uid"] == "959"
        # returned dict mirrors the re-selected DB row (original_folder = INBOX)
        assert out["original_folder"] == "INBOX"
        assert len(db.inserted) == 1
        table, cols, values = db.inserted[0]
        assert table == "sogo6_snoozed"
        assert values[0][4] == "Archive"  # target_folder

    def test_success_original_folder_defaults_to_folder(self):
        db = _SnoozeDb(select_results=[[], []])
        module = ModuleSnooze(db)
        future = _DateTime.now(_dt_tz.utc) + timedelta(days=1)
        out = module.snooze("u", "0", "959", "INBOX", future)
        assert out["original_folder"] == "INBOX"
        assert db.inserted[0][2][0][4] == "INBOX"

    def test_success_no_reselect_rows_fallback_dict(self):
        db = _SnoozeDb(select_results=[[], []])
        module = ModuleSnooze(db)
        future = _DateTime.now(_dt_tz.utc) + timedelta(days=1)
        out = module.snooze("u", "0", "959", "SPAM", future)
        assert out["user_uid"] == "u"
        assert out["folder"] == "SPAM"
        assert out["snooze_until"] == future.isoformat()


class TestListSnoozed:
    def test_include_expired_returns_all(self):
        row = _snooze_row(snooze_until=_DateTime(2020, 1, 1, tzinfo=_dt_tz.utc))
        db = _SnoozeDb(select_results=[[row]])
        module = ModuleSnooze(db)
        out = module.list_snoozed("u", include_expired=True)
        assert len(out) == 1

    def test_filters_expired_by_default(self):
        row = _snooze_row(snooze_until=_DateTime(2020, 1, 1, tzinfo=_dt_tz.utc))
        db = _SnoozeDb(select_results=[[row]])
        module = ModuleSnooze(db)
        out = module.list_snoozed("u")
        assert out == []

    def test_filters_naive_expired(self):
        row = _snooze_row(snooze_until=_DateTime(2020, 1, 1))
        db = _SnoozeDb(select_results=[[row]])
        module = ModuleSnooze(db)
        assert module.list_snoozed("u") == []

    def test_keeps_future(self):
        row = _snooze_row(snooze_until=_DateTime(2035, 1, 1, tzinfo=_dt_tz.utc))
        db = _SnoozeDb(select_results=[[row]])
        module = ModuleSnooze(db)
        out = module.list_snoozed("u")
        assert len(out) == 1
        assert out[0]["mail_uid"] == "959"

    def test_none_snooze_until_filtered(self):
        row = _snooze_row(snooze_until=None)  # explicit None stays None
        db = _SnoozeDb(select_results=[[row]])
        module = ModuleSnooze(db)
        assert module.list_snoozed("u") == []
...
class TestListDue:
    def test_with_explicit_now(self):
        ...
        db = _SnoozeDb(select_results=[[row]])
        module = ModuleSnooze(db)
        now = _DateTime(2026, 9, 1, tzinfo=_dt_tz.utc)
        out = module.list_due(now)
        assert out[0]["mail_uid"] == "959"
        assert db.select_calls[0][0] == "sogo6_snoozed"

    def test_default_now_empty(self):
        db = _SnoozeDb()
        module = ModuleSnooze(db)
        assert module.list_due() == []


class TestParsePreset:
    def test_valid_presets(self):
        assert ModuleSnooze.parse_preset("later_today") == {"hours": 3}
        assert ModuleSnooze.parse_preset("next_week") == {"days": 7}

    def test_invalid_preset_returns_none(self):
        assert ModuleSnooze.parse_preset("whenever") is None


class TestRemoveRecord:
    def test_remove_record_deletes(self):
        db = _SnoozeDb()
        module = ModuleSnooze(db)
        module.remove_record(7)
        assert db.deleted[0][0] == "sogo6_snoozed"


class TestUnsnooze:
    def test_unsnooze_deletes_and_returns_record(self):
        row = _snooze_row(mail_uid="959", folder="INBOX")
        db = _SnoozeDb(select_results=[[row]])
        module = ModuleSnooze(db)
        out = module.unsnooze("testuser@example.org", 1)
        assert out["mail_uid"] == "959"
        assert out["original_folder"] == "INBOX"
        assert db.deleted[0][0] == "sogo6_snoozed"

    def test_unsnooze_missing_record_raises_not_found(self):
        db = _SnoozeDb()
        module = ModuleSnooze(db)
        with pytest.raises(RequestException) as ex:
            module.unsnooze("testuser@example.org", 999)
        assert ex.value.error == err.ERROR_SNOOZE_NOT_FOUND
        assert db.deleted == []
